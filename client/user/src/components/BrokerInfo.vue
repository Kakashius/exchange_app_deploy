...
export default {
	...
	created() {
		...
		const user = this.$store.state.brokers.find((b) => b.id === +this.$props.id);
		this.user.name = user.name;

		this.unsubscribe = this.$store.subscribe((mutation) => {
			if (mutation.type === 'add') {
				this.user.money = this.$store.state.brokers.find((t) => t.id === +this.$props.id).money;
				...
				this.user.stocks = this.$store.state.trades
					.filter((t) => t.uid === +this.$props.id && t.amount > 0)
					.map((t) => {
